...
export function BookAppointmentTool({ apiKey, subdomain }: { apiKey: string; subdomain: string }) {
  ...
  const retreatFocus = useCallback(() => {
    setFocus((f: BookingFocus) => {
      const idx = FOCUS_CYCLE.indexOf(f);
      return FOCUS_CYCLE[(idx - 1 + FOCUS_CYCLE.length) % FOCUS_CYCLE.length] ?? "location";
    });
  }, []);
  ...
  return (
    <box flexDirection="column" flexGrow={1}>
      ...
      >
        <Field label="Location ID"  width={13} value={locationId}  focused={focus === "location"}  placeholder="123"        onInput={setLocationId}  onSubmit={() => nextFocus("patient")} />
        <Field label="Patient ID"   width={11} value={patientId}   focused={focus === "patient"}   placeholder="456"        onInput={setPatientId}   onSubmit={() => nextFocus("date")} />
        <Field label="Start Date"   width={16} value={startDate}   focused={focus === "date"}      placeholder="YYYY-MM-DD" onInput={setStartDate}   onSubmit={() => nextFocus("time")} />
        <Field label="Start Time"   width={12} value={startTime}   focused={focus === "time"}      placeholder="HH:MM"      onInput={setStartTime}   onSubmit={() => nextFocus("duration")} />
        <Field label="Dur (min)"    width={9}  value={durationStr} focused={focus === "duration"}  placeholder="60"         onInput={setDurationStr} onSubmit={() => nextFocus("note")} />
      ...
      <box
        ...
        style={{ flexShrink: 0, gap: 1, paddingLeft: 1, paddingRight: 1 }}
      >
        <Field label="Note"          value={note}          focused={focus === "note"}          placeholder="Appointment note…" onInput={setNote}          onSubmit={() => nextFocus("referrer")}      fg="#565f89" />
        <Field label="Referrer"      width={13} value={referrer}      focused={focus === "referrer"}      placeholder="referrer"         onInput={setReferrer}      onSubmit={() => nextFocus("descriptorIds")} fg="#565f89" />
        <Field label="Descriptor IDs" width={16} value={descriptorIds} focused={focus === "descriptorIds"} placeholder="1,2,3"            onInput={setDescriptorIds} onSubmit={() => nextFocus("apptPerSlot")}   fg="#565f89" />
        <Field label="Appts/Slot"    width={10} value={apptPerSlot}  focused={focus === "apptPerSlot"}  placeholder="1"                onInput={setApptPerSlot}  onSubmit={() => nextFocus("notifyPatient")} fg="#565f89" />
        <Field label="Notify Pt"     width={9}  value={notifyPatient}  focused={focus === "notifyPatient"}  placeholder="t/f" onInput={setNotifyPatient}  onSubmit={() => nextFocus("notifyPractice")} fg="#565f89" />
        <Field label="Notify Pr"     width={9}  value={notifyPractice} focused={focus === "notifyPractice"} placeholder="t/f" onInput={setNotifyPractice} onSubmit={() => nextFocus("unavailable")}    fg="#565f89" />
        <Field label="Unavail"       width={8}  value={unavailable}    focused={focus === "unavailable"}    placeholder="t/f" onInput={setUnavailable}    onSubmit={() => nextFocus("isGuardian")}     fg="#565f89" />
        <Field label="Guardian"      width={9}  value={isGuardian}     focused={focus === "isGuardian"}     placeholder="t/f" onInput={setIsGuardian}     onSubmit={() => nextFocus("isNewClient")}    fg="#565f89" />
        <Field label="New Client"    width={10} value={isNewClient}    focused={focus === "isNewClient"}    placeholder="t/f" onInput={setIsNewClient}    onSubmit={() => nextFocus("providers")}      fg="#565f89" />
      </box>
      ...
      <box flexDirection="row" flexGrow={1}>
        ...
        <box title=" Providers " border borderStyle="single"
          borderColor={focus === "providers" ? "#7aa2f7" : "#414868"}
          style={{ flexGrow: 1 }}
        >
          <select
        ...
        <box title=" Operatories " border borderStyle="single"
          borderColor={focus === "operatories" ? "#7aa2f7" : "#414868"}
          style={{ flexGrow: 1 }}
        >
          <select
        ...
        <box title=" Appt Types " border borderStyle="single"
          borderColor={focus === "apptTypes" ? "#7aa2f7" : "#414868"}
          style={{ flexGrow: 1 }}
        >
          <select
      ...
      >
        <scrollbox focused={false} style={{ flexGrow: 1 }}>
          ...
          {!loading && !fetchLoading && result      && <code content={result} filetype="json" syntaxStyle={JSON_SYNTAX_STYLE} />}
...
type Tool = {
  name:      string;
  component: React.FC<{ apiKey: string; subdomain: string }>;
};
...
export function ToolsScreen({
  ...
  const [toolsFocus, setToolsFocus] = useState<ToolsFocus>("tool");

  useKeyboard((key) => {
    if (key.ctrl && key.name === "t") { onSwitchToExplorer(); return; }
    if (toolsFocus === "list") {
      if (key.name === "tab")   { setToolsFocus("tool"); return; }
  ...
  return (
    <box flexDirection="column" flexGrow={1}>
      ...
      >
        ...
        <text fg="#414868">  |  </text>
        <text fg="#565f89">[Tab] next field  [Ctrl+L] fetch  [Ctrl+R] book  [Ctrl+T] explorer  [Ctrl+C] quit</text>
      </box>
      ...
      <box flexDirection="row" flexGrow={1}>
        ...
        {/* Active tool */}
        {React.createElement(selected.component, { apiKey, subdomain })}
